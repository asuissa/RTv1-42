...
/*   Updated: 2019/04/13 12:00:35 by ymekraou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_diffuse(double res[3], double light_vector[3],
			t_light *light, t_hit *hit_point)
{
	double		light_intensity;
	double		diffuse_att;
	double		light_red;
	...
	if (diffuse_att > 0)
	{
		light_intensity = compute_light_intensity(hit_point->coord,
				light->pos_relative, light->power);
		...
		res[0] += light_red * (hit_point->red / 255.0)
				* hit_point->diffuse_coeff;
		res[1] += light_green * (hit_point->green / 255.0)
				* hit_point->diffuse_coeff;
		res[2] += light_blue * (hit_point->blue / 255.0)
				* hit_point->diffuse_coeff;
	}
}

void	add_specular(double res[3], double angle,
			t_light *light, t_hit *hit_point)
{
	double		light_intensity;
	double		specular_att;
	...
	light_intensity = compute_light_intensity(hit_point->coord,
			light->pos_relative, light->power);
	res[0] += hit_point->specular_coeff * (double)(light->red)
			* specular_att * light_intensity;
	res[1] += hit_point->specular_coeff * (double)(light->green)
			* specular_att * light_intensity;
	res[2] += hit_point->specular_coeff * (double)(light->blue)
			* specular_att * light_intensity;
}
